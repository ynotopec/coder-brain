...
test('DirectReplier fallback answers the capital of France in French', async () => {
  const llm = new FakeLLM(['not-json', 'still-not-json']);
  const replier = new DirectReplier(llm);

  const reply = await replier.generateChatResponse('Quelle est la capitale de la France ?');

  assert.equal(reply.message, 'La capitale de la France est Paris.');
  assert.deepEqual(reply.topic_continuation, ['géographie', 'culture française']);
  assert.equal(llm.callCount, 2);
});


test('DirectReplier fallback handles French typo variant for capital question', async () => {
  const llm = new FakeLLM(['not-json', 'still-not-json']);
  const replier = new DirectReplier(llm);
