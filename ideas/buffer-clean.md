...
flowchart TD
  ...
  subgraph ENGINE [Phase 2: Execution Engines]
    ...
    RISK -->|Safe| EXEC[Execute Tool]:::logic
    HITL -->|Approved| EXEC
    EXEC --> VERIFY{Verify Output}:::gate
    VERIFY -->|Fail| ROLL[Rollback / Compensate]:::logic

    %% HYBRID FLOW
    ROUTER -->|Hybrid| HYB[Hybrid Orchestrator]:::logic
    HYB --> R_PLAN
    HYB --> A_PLAN
  ...
  subgraph REVIEW [Phase 3: System 2 Evaluation]
    GEN --> AGG[Response Aggregator]:::logic
    FALL --> AGG
    EXEC --> AGG
    ROLL --> AGG
    ...
    F_C -.-> SCORE[Weighted Quality Score]:::logic
    T_C -.-> SCORE
    SCORE --> QUAL_PASS{Quality Pass?}:::gate
  end

  %% --- PHASE 4: OUTPUT & LOOP ---
  POL_PASS -->|No| FORCE_EXIT[Generate Apology]:::logic
  QUAL_PASS -->|No| RETRY{Max Retries?}:::gate
  RETRY -->|Limit Reached| FORCE_EXIT
  RETRY -->|Under Limit| REFINE[Refinement Manager]:::logic
  REFINE -->|Inject Error Context| BUILD

  %% Fast Track (with lightweight safety check)
  CHAT --> CHAT_SAFE[Light Safety Check]:::crit
  CHAT_SAFE --> CHAT_PASS{Safe?}:::gate
  CHAT_PASS -->|No| FORCE_EXIT
  CHAT_PASS -->|Yes| FINAL

  POL_PASS -->|Yes| QUAL_PASS
  QUAL_PASS -->|Yes| FINAL([Final Response]):::term
  FORCE_EXIT --> FINAL
  ...
  MEM_POL -->|No| MEM_SKIP[Do Not Persist]:::logic
  MEM_WRITE --> MEM

  %% --- OBSERVABILITY ---
  OBS[(Observability: logs/metrics/traces)]:::db
  EXEC -.-> OBS
  ROLL -.-> OBS
  FORCE_EXIT -.-> OBS
  FINAL -.-> OBS
